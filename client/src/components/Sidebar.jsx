  ...
  return (
    ...
    {chats.length>0 && <p className="mt-4 text-sm">Recent Chats</p>}
      <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
        {
            chats.filter((chats)=> chats.messages[0] ? chats.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chats.name.toLowerCase().includes(search.toLowerCase())).map((chats)=>(
                <div onClick={()=> {navigate('/'); setSelectChat(chats); setIsMenuOpen(false)}} key={chats.id} className="p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group  ">
                    <div>
      ...
      <div className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group">
        ...
        {user && <img className="h-5 cursor-pointer hidden not-dark:invert group-hover:block" src={assets.logout_icon}/>}
      </div>
