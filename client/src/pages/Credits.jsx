        ...
        <div className='flex flex-wrap justify-center gap-8'>
            {plans.map((plans)=>(
              <div key={plans._id} className={`border border-gray-200 dark:border-purple-700 rounded-lg shadow hover:shadow-lg transition-shadow p-6 min-w-[300px] flex flex-col ${plans._id === "pro" ? "bg-purple-50 dark:bg-purple-900" : "bg-white dark:bg-transparent"}`}>
                ...
                <button onClick={() => toast.promise(purchasePlan(plans._id), {loading: 'Processing...'})} className='mt-6 bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white font-medium py-2 rounded transition-colors cursor-pointer'>Buy Now</button>
              </div>
            ))}
        </div>
    </div>
